...
from .enums import DnsOpCode, DnsQR, DnsResponseCode
...
class SQLDatabase(Database):
    def __init__(self, database="dns", user=None, password=None, host="localhost", port=3306):
        if mysql is None:
            raise RuntimeError("mysql.connector is required to use SQLDatabase")
    ...
    def get_blob_id(self, blob):
        with closing(self.db.cursor()) as cursor:
            ...
            cursor.execute(
                "INSERT IGNORE INTO dns.blob (`sha1`, `blob`) VALUES (%s, %s)", (sha1, blob)
            )
            self.queries += 1
            self.db.commit()
            return sha1
    ...
    def get_name(self, name_id):
        with closing(self.db.cursor()) as cursor:
            sequence = []
            for _foo in range(256):  # dns labels are limited to 256 depths anyway
    ...
    def create_name(self, label, parent, casemap=str.upper):
        ...
        with closing(self.db.cursor()) as cursor:
            cursor.execute(
                "INSERT INTO name (`name`, `parent`) VALUES (%s, %s)", (casemap(label), parent)
            )
            self.queries += 1
            self.db.commit()
            self.log.debug(
                "create_name(%s,%s) created with id: %d" % (label, parent, cursor.lastrowid)
            )
            return cursor.lastrowid
    ...
    def create_resource_header(self, resource):
        ...
        with closing(self.db.cursor()) as cursor:
            cursor.execute(
                "INSERT INTO resource_header (`name`, `type`, `class`) VALUES (%s, %s, %s)",
                (name_id, resource_type, resource_class),
            )
            self.queries += 1
            self.db.commit()
            self.log.debug(
                "create_resource_header(%s,%s,%s) created with id: %d"
        ...
        with closing(self.db.cursor()) as cursor:
            ...
            for label in reversed(name):
                with closing(self.db.cursor()) as cursor:
                    if node:
                        cursor.execute(
                            "SELECT id FROM name WHERE name=%s AND parent=%s LIMIT 1", (label, node)
                        )  # %d
                    else:
                        if not label:
                            label = "."
                        cursor.execute(
                            "SELECT id FROM name WHERE name=%s AND parent IS NULL LIMIT 1", (label,)
                        )  # (label,) is a tuple (label) is a string...
    ...
    def create_query(self, packet_id, ns_id=None, addr_id=None, parent_id=None, response_id=None):
        with closing(self.db.cursor()) as cursor:
            ...
            self.db.commit()
            self.log.debug(
                "create_query(%s,%s,%s,%s,%s) created with id: %d"
    ...
    def update_query_response(self, query_id, response_id):
        with closing(self.db.cursor()) as cursor:
            cursor.execute(
                "UPDATE query SET `response` = %s WHERE `packet` = %s LIMIT 1",
                (response_id, query_id),
            )
            self.queries += 1
            self.db.commit()
            self.log.debug("update_query_response(%s,%s)" % (query_id, response_id))
    ...
    def get_record_id(self, record):
        ...
        with closing(self.db.cursor()) as cursor:
            cursor.execute(
                "SELECT id FROM resource_record WHERE `header`=%s AND `rdata`=%s LIMIT 1",
    ...
    def get_resource_header_id(self, resource):
        ...
        with closing(self.db.cursor()) as cursor:
            cursor.execute(
                "SELECT id FROM resource_header WHERE `name`=%s AND `type`=%s AND `class`=%s LIMIT 1",
                (name_id, resource_type, resource_class),
            )
            self.queries += 1
            r = cursor.fetchone()
    ...
    def create_record(self, record):
        ...
        with closing(self.db.cursor()) as cursor:
            cursor.execute(
                "INSERT INTO `resource_record` (`header`, `rdata`) VALUES (%s, %s)",
    ...
    def get_packet_records(self, packet_id):
        ...
        with closing(self.db.cursor()) as cursor:
            ...
            for row in rows:
                ...
                record = DnsRecord(name, rtype, rclass, ttl, rdlength=len(rdata), rdata=rdata)
    ...
    def get_questionset_id(self, questions):
        ...
        return self._get_digest(
            ",".join(map(str, (sorted(map(self.get_resource_header_id, questions))))).encode(
                "ascii"
            )
        )
    ...
    def get_recordset_id(self, records):
        ...
        return self._get_digest(
            ",".join(map(str, (sorted(map(self.get_record_id, records))))).encode("ascii")
        )
    ...
    def create_packet_record(self, packet_id, record):
        ...
        if (
            hasattr(record.name, "compressed_name")
            and record.name.encode() != record.name.compressed_name
        ):
    ...
    def create_packet_question(self, packet_id, question):
        ...
        if (
            hasattr(question.name, "compressed_name")
            and question.name.encode() != question.name.compressed_name
        ):
